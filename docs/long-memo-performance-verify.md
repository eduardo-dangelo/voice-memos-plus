...
6. **Unmute / clear solo mid-play** — mute one layer, Play, unmute mid-take; newly unmuted layer must sound via decode + additive arm (no full stop/restart); playhead keeps moving
...
## Expected
- Listen-play still full-decodes **audible** stems only (muted/soloed-out stay cold until unmute; unmute arms into the live session without tearing down the transport clock)
